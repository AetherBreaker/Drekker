...
# First party imports
from drekker.logging.logging_bases import FixedFormatter, FixedLogRecord
...
if TYPE_CHECKING:
  ...
  # First party imports
  from drekker.logging import logging_config  # noqa: PLW0406
...
def configure_logging_main(  # noqa: PLR0915
  ...
  if logging_type == "per_run":
    ...
    # First party imports
    from drekker.logging.logging_bases import CustomTimedRotatingFileHandler
  ...
  textual_console_handler.setFormatter(preferred_formatter)

  handlers: list[logging.Handler] = [debug_file_handler, info_file_handler]

  textual_console_handler.setLevel(logging.INFO)
  if queue_console_handler:
    handlers.append(textual_console_handler)
  else:
